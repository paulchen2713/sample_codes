# -*- coding: utf-8 -*-
"""
Created on Tue Oct 05 9:55:20 2022

@patch: 2022.10.05 
@author: Paul
@file: visualize_signal.py
@dependencies:
    envs        test
    python      3.6.13
    ipython     7.16.1
...
class SignalVisualizer():
    ...
    def save_scale(self, path, signal_type='range_doppler', color_scale=None,
        ...
        def test_plot():
            plt.plasma()
            plt.imshow(self.image, extent=[0, 1, 0, 1])
            plt.show()
        
        # test_plot()

        # img = self._format_img(self.image, signal_type, color_scale, rotation)
        # (self, img, signal_type, color_scale, rotation)
        img = self._format_img(img=self.image, signal_type=signal_type, color_scale=color_scale, rotation=rotation)
        print(f"img: {img}") # Figure(700x1200)
        if save_img:
            print(f"save img")
            plt.savefig(path)
        if plot_img:
    ...
    def _format_img(self, img, signal_type, color_scale, rotation):
        ...
        if signal_type == 'range_doppler':
            fig, ax = plt.subplots(figsize=(7, 12))
            # fig, ax = plt.subplots(figsize=(2.56, 0.64))
