...
get_shape(rd_matrix)
np.savetxt(CURR_PATH + 'logs/000117_256_64.txt', rd_matrix, delimiter=' ')



# plot the matric with its original shape, rd_matrix.shape = (256, 64)
# plt.imshow(rd_matrix) 
plt.matshow(rd_matrix, interpolation="nearest")
plt.plasma()
# plt.axis('off')
plt.savefig(CURR_PATH + 'figs/rd_matrix.png', bbox_inches='tight', pad_inches=0)

rrdm = np.rot90(rd_matrix, k=2) # rotate 180 degrees to fit cartesian coordinates
plt.matshow(rrdm, interpolation="nearest")
plt.plasma()
# plt.axis('off')
plt.savefig(CURR_PATH + 'figs/rrdm.png', bbox_inches='tight', pad_inches= 0)


# plt.xlim(-13.5, 13.5)
# plt.ylim(0, 50)
plt.savefig(CURR_PATH + 'figs/000117_256_64.png')
